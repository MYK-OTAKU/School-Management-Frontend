...
const UserForm = ({ user, onClose, roles = [] }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.username.trim()) {
      ...
      newErrors.username = getTranslation('usersForm.usernameTooShort', 'Le nom d\'utilisateur doit contenir au moins 3 caractères');
    }
    ...
    if (!isEdit || formData.password) {
      if (!isEdit && !formData.password) {
        ...
        newErrors.password = getTranslation('usersForm.passwordTooShort', 'Le mot de passe doit contenir au moins 6 caractères');
      }
    ...
    if (!formData.firstName.trim()) {
      newErrors.firstName = getTranslation('usersForm.firstNameRequired', 'Le prénom est requis');
    }
    ...
    if (formData.phone && formData.phone.trim()) {
      ...
      if (!phoneRegex.test(formData.phone)) {
        newErrors.phone = getTranslation('usersForm.invalidPhone', 'Format de téléphone invalide');
      }
    }
    
    // Validation du rôle
    if (!formData.roleId) {
      newErrors.roleId = getTranslation('usersForm.roleRequired', 'Le rôle est requis');
    } else {
      ...
      if (selectedRole && !canAssignRole(selectedRole.name)) {
        newErrors.roleId = getTranslation('usersForm.roleNotAllowed', 'Vous ne pouvez pas attribuer ce rôle');
      }
    }
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className={`block mb-2 text-sm font-medium ${getTextColorClass(false)}`}>
                  {getTranslation('usersForm.firstName', 'Prénom')} *
                </label>
                <input
                  type="text"
                  name="firstName"
                  ...
                  placeholder={getTranslation('usersForm.firstNameLabel', 'Prénom')}
                  disabled={isSubmitting}
                />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className={`block mb-2 text-sm font-medium ${getTextColorClass(false)}`}>
                  {getTranslation('usersForm.phone', 'Téléphone')}
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  ...
                  <input
                    ...
                    placeholder={getTranslation('usersForm.phoneLabel', 'Téléphone')}
                    disabled={isSubmitting}
                  />
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    // ✅ i18n: Placeholder conditionnel avec getTranslation
            ...
            <div>
              <label className={`block mb-2 text-sm font-medium ${getTextColorClass(false)}`}>
                {getTranslation('usersForm.role', 'Rôle')} *
              </label>
              {/* ✅ i18n: Avertissement traduit */}
              {filteredRoles.length === 0 && (
                <div className={`mb-2 p-2 rounded ${getWarningBgClass()} ${getWarningBorderClass()} ${getWarningColorClass()} text-sm`}>
                  {getTranslation('usersForm.noRolesAvailable', 'Aucun rôle disponible pour votre niveau d\'autorisation')}
                </div>
              )}
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                >
                  <option value="" className={isDarkMode ? 'bg-gray-800 text-white' : 'bg-[var(--background-primary)] text-[var(--text-primary)]'}>{getTranslation('usersForm.selectRole', 'Sélectionner un rôle')}</option>
              ...
              {filteredRoles.length > 0 && (
                <div className={`mt-1 text-xs ${getTextColorClass(false)}`}>
                  {/* ✅ i18n: Interpolation pour info hiérarchique */}
                  {getTranslation('usersForm.roleHierarchyInfo', 'Votre rôle')}"{currentUser?.role?.name}" {getTranslation('usersForm.roleHierarchyManage', 'vous permet de gérer')} : {filteredRoles.map(r => getTranslation(`usersForm.roleNames.${r.name}`, r.name)).join(', ')}
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="twoFactorEnabled" className={`ml-2 block text-sm ${getTextColorClass(false)}`}>
                  {getTranslation('usersForm.twoFactorEnabled', 'Authentification à deux facteurs')}
                </label>
              </div>
            </div>
            ...
            <div className={`flex justify-end space-x-3 pt-4 border-t ${getBorderColorClass()}`}>
              ...
              >
                {isSubmitting ? (
                  ...
                  isEdit ? getTranslation('common.update', 'Mettre à jour') : getTranslation('common.create', 'Créer')
                )}
              </button>
            </div>
          </form>
