...
const Users = () => {
  ...
  const handleToggleStatus = (user) => {
    ...
    setConfirmDialog({
      ...
      type: 'toggleStatus',
      user,
      // ✅ i18n: Utilise getTranslation pour titres dynamiques
      title: user.isActive ? 
        getTranslation('users.deactivateUser', 'Désactiver l\'utilisateur') : 
        getTranslation('users.activateUser', 'Activer l\'utilisateur'),
      // ✅ i18n: Interpolation simple pour message dynamique (clean code: pas de getTranslationWithVars ici, car pas de vars complexes)
  ...
  const handleDelete = (user) => {
    ...
    setConfirmDialog({
      ...
      // ✅ i18n: Interpolation pour message
    ...
    <div className="space-y-6">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            <option value="" className={isDarkMode ? 'bg-gray-800 text-white' : 'bg-[var(--background-primary)] text-[var(--text-primary)]'}>{getTranslation('users.allRoles', 'Tous les rôles')}</option>
            {roles?.map(role => (
      ...
      >
        {filteredUsers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className={`${getTextColorClass(false)} text-lg`}>{getTranslation('users.noUsersFound', 'Aucun utilisateur trouvé')}</p>
