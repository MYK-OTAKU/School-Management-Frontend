...
const PermissionForm = ({ permission, onClose }) => {
  // ✅ i18n: Destructuration complète pour inclure getTranslationWithVars (correction de l'erreur ReferenceError)
  const { getTranslation, getTranslationWithVars } = useLanguage();
  ...
  const title = isEdit ? 
    getTranslation('permissionsForm.editPermission', 'Modifier la permission') : 
    getTranslation('permissionsForm.addPermission', 'Ajouter une permission');
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      errors.name = getTranslation('permissionsForm.nameRequired', 'Le nom est requis');
    } else if (formData.name.length < 2) {
      errors.name = getTranslation('permissionsForm.nameTooShort', 'Le nom doit contenir au moins 2 caractères');
    }

    if (!formData.description.trim()) {
      errors.description = getTranslation('permissionsForm.descriptionRequired', 'La description est requise');
    }
              ...
              <div>
                ...
                {isCritical && (
                  <div className="flex items-center space-x-1 mt-1">
                    ...
                    <span className={`text-sm ${getWarningColorClass()}`}>{getTranslation('permissionsForm.systemProtected', 'Permission système protégée')}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className={`p-3 rounded-md border ${getWarningBgClass()} ${getWarningBorderClass()} ${getWarningColorClass()}`}>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm">
                  {getTranslation('permissionsForm.systemCannotModify', 'Cette permission système ne peut pas être modifiée pour des raisons de sécurité.')}
                </span>
              </div>
            </div>
          )}
          ...
          <div>
            <label htmlFor="name" className={`block text-sm font-medium mb-2 ${getTextColorClass(false)}`}>
              {getTranslation('permissionsForm.permissionName', 'Nom de la permission')} *
            </label>
            <input
              type="text"
              id="name"
              ...
              // ✅ i18n: Placeholder traduit (exemple statique conservé pour clarté technique)
            ...
            {/* ✅ i18n: Texte d'aide traduit */}
            <p className={`mt-1 text-xs ${getTextColorClass(false)}`}>
              {getTranslation('permissionsForm.nameFormatHelp', 'Format recommandé: RESOURCE_ACTION (ex: USERS_VIEW, POSTS_MANAGE)')}
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              // ✅ i18n: Placeholder traduit
              placeholder={getTranslation('permissionsForm.permissionDescriptionPlaceholder', 'Description de la permission...')}
              maxLength={255}
            />
            {validationErrors.description && (
              <p className={`mt-1 text-sm ${getErrorColorClass()}`}>{validationErrors.description}</p>
            )}
            {/* ✅ i18n: Compteur traduit avec interpolation (utilise getTranslationWithVars) */}
            <p className={`mt-1 text-xs ${getTextColorClass(false)}`}>
              {getTranslationWithVars('permissionsForm.charactersCount', { length: formData.description.length, max: 255 }, '{length}/{max} caractères')}
            </p>
          </div>
          ...
          {!isEdit && (
            <div className={`p-3 rounded-md border ${getInfoBgClass()} ${getInfoBorderClass()}`}>
              <h4 className={`text-sm font-medium ${getInfoColorClass()} mb-2`}>{getTranslation('permissionsForm.examplesTitle', 'Exemples de permissions :')}</h4>
              <div className={`text-xs ${getTextColorClass(false)} space-y-1`}>
                <div>• <span className={`${getInfoColorClass()}`}>USERS_VIEW</span> - {getTranslation('permissionsForm.exampleUsersView', 'Voir les utilisateurs')}</div>
                <div>• <span className={`${getInfoColorClass()}`}>POSTS_MANAGE</span> - {getTranslation('permissionsForm.examplePostsManage', 'Gérer les publications')}</div>
                <div>• <span className={`${getInfoColorClass()}`}>REPORTS_EXPORT</span> - {getTranslation('permissionsForm.exampleReportsExport', 'Exporter les rapports')}</div>
              </div>
            </div>
          )}
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    <span>{isEdit ? getTranslation('permissionsForm.processingUpdate', 'Modification...') : getTranslation('permissionsForm.processingCreate', 'Création...')}</span>
                  </>
                ) : (
                  <>
                    <Save size={16} />
                    <span>{isEdit ? getTranslation('common.update', 'Modifier') : getTranslation('common.create', 'Créer')}</span>
                  </>
                )}
              </button>
            )}
